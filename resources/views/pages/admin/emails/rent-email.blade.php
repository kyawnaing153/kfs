...
| **{{ $item->productVariant->product->product_name }}**<br><small style="color: #6c757d;">Size: {{ $item->productVariant->size }}</small> | {{ $item->rent_qty }} | {{ number_format($item->unit_price, 0) }} Ks | {{ number_format($item->total, 0) }} Ks |
@endforeach
</x-mail::table>
...
| | |
|:---|---:|
| **Subtotal** | {{ number_format($rent->sub_total, 0) }} Ks |
| **Transportation** | {{ number_format($rent->transport, 0) }} Ks |
| **Security Deposit** | {{ number_format($rent->deposit, 0) }} Ks |
@if($rent->discount > 0)
| **Discount** | <span style="color: #28a745;">-{{ number_format($rent->discount, 0) }} Ks</span> |
@endif
| **Grand Total** | **{{ number_format($rent->total, 0) }} Ks** |
</x-mail::table>
...
<div style="background-color: #f8f9fa; padding: 16px; border-radius: 8px;">
    <table style="width: 100%;">
        <tr>
            ...
            <td style="padding: 8px 0; text-align: right;">{{ number_format($rent->total, 0) }} Ks</td>
        </tr>
        <tr>
            <td style="padding: 8px 0;"><strong>Amount Paid:</strong></td>
            <td style="padding: 8px 0; text-align: right; color: #28a745;">{{ number_format($rent->total_paid, 0) }} Ks</td>
        </tr>
        <tr style="border-top: 2px solid #dee2e6;">
            ...
            <td style="padding: 12px 0 8px 0; text-align: right;">
                <strong style="font-size: 20px; color: #dc3545;">{{ number_format($rent->total_due, 0) }} Ks</strong>
            </td>
        </tr>
    </table>
</div>
