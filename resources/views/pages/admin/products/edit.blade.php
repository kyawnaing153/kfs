...
@section('content')
    ...
    <div class="grid grid-cols-1 gap-6 xl:grid-cols-1">
        <x-common.component-card title="Edit Product Information">
            <form method="POST" action="{{ route('products.update', $product->id) }}" enctype="multipart/form-data">
                    ...
                    <div>
                        ...
                        <div x-data="{
                            ...
                            <div @drop.prevent="handleDrop($event)" @dragover.prevent="isDragging = true"
                                ...
                                <div class="flex flex-col items-center m-0">
                                    ...
                                    <h4 class="mb-3 font-semibold text-gray-800 text-theme-xl dark:text-white/90">
                                        <span x-show="!isDragging">Update Thumbnail</span>
                            ...
                            <div x-show="files.length > 0" class="mt-4 p-4 border-t border-gray-200 dark:border-gray-700"
                                x-cloak>
                                <h5 class="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">New Thumbnail Preview:</h5>
                                <ul class="space-y-2">
                        ...
                        <div>
                            ...
                            <div class="flex items-center">
                                <label class="relative inline-flex items-center cursor-pointer">
                                    ...
                                    <input type="checkbox" name="is_feature" value="1" {{ old('is_feature', $product->is_feature) == '1' ? 'checked' : '' }}
                                        class="sr-only peer">
                ...
                @if($product->variants->isNotEmpty())
                    <div class="mt-6 rounded-lg bg-gray-50 p-4 dark:bg-gray-800/50">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                                    This product has {{ $product->variants->count() }} variant(s) with {{ $product->variants->sum('qty') }} total stock.
                                </p>
                            </div>
                            <a href="{{ route('products.variants.manage', $product->id) }}"
                                class="inline-flex items-center gap-2 rounded-lg border border-gray-300 bg-white px-3 py-1.5 text-xs font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700">
                                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <div class="mt-3 grid grid-cols-2 gap-4 sm:grid-cols-3">
                            <div>
                                <p class="text-xs text-gray-500 dark:text-gray-400">Total Variants</p>
                            ...
                            <div>
                                <p class="text-xs text-gray-500 dark:text-gray-400">Variant Types</p>
                                <p class="text-sm font-medium text-gray-800 dark:text-white">
                                    {{ $product->variants->pluck('size')->filter()->unique()->count() }} sizes
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex">
                        ...
                        <div class="ml-3">
                            ...
                            <div class="mt-2 text-sm text-blue-700 dark:text-blue-400">
                                <p>
                                    You can manage product variants (sizes, colors, etc.) and set pricing 
                                    for each variant (sale price and/or rental prices with different durations)
                                    from the variants management page.
                                </p>
                                @if($product->variants->isEmpty())
                                    <p class="mt-1 font-medium">
                                        This product has no variants yet. Add variants after updating the product.
                                    </p>
                                @endif
                            </div>
                        </div>
                ...
                <div class="mt-8 flex flex-wrap gap-3">
                    ...
                    <a href="{{ route('products.variants.manage', $product->id) }}"
                        class="inline-flex items-center gap-2 rounded-lg border border-blue-300 bg-blue-50 px-4 py-2.5 text-sm font-medium text-blue-700 hover:bg-blue-100 focus:outline-none focus:ring-2 focus:ring-blue-500/30 dark:border-blue-700 dark:bg-blue-900/30 dark:text-blue-400 dark:hover:bg-blue-900/50">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
