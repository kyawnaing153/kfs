...
@section('content')
    <x-common.page-breadcrumb pageTitle="Edit Staff: {{ $staff->name }}" />

    <div class="grid grid-cols-1 gap-6 xl:grid-cols-1">
        <x-common.component-card title="Edit Staff Information">
            <form method="POST" action="{{ route('staffs.update', $staff->id) }}" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                    ...
                    <div>
                        ...
                        <input type="tel" name="phone_number" value="{{ old('phone_number', $staff->phone_number) }}"
                            class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30"
                            required />
                        @error('phone_number')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                ...
                <div class="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
                    ...
                    <div>
                        ...
                        <input type="number" name="salary" value="{{ old('salary', $staff->salary) }}" step="0.01"
                            min="0"
                            class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30"
                            required />
                        @error('salary')
                ...
                <div class="mt-4 grid grid-cols-1 gap-4">
                    <div>
                        ...
                        <textarea name="address" rows="3"
                            class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30">{{ old('address', $staff->address) }}</textarea>
                        @error('address')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <!-- Row 4: Current Profile Picture & Status -->
                <div class="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                <p class="text-sm text-gray-600 dark:text-gray-400">
                                    {{ $staff->profile_picture ? 'Current image' : 'No image uploaded' }}
                                </p>
                                @if ($staff->profile_picture)
                                    <button type="button"
                                        @click="if(confirm('Are you sure you want to remove the profile picture?')) document.getElementById('remove_profile_picture').value = '1'; document.getElementById('current_profile_image').classList.add('hidden');"
                        ...
                        <!-- Hidden field to indicate profile picture removal -->
                    ...
                    <div>
                        ...
                        <select name="status"
                            ...
                            required>
                            <option value="1" {{ old('status', $staff->status) == '1' ? 'selected' : '' }}>Active
                            </option>
                            <option value="0" {{ old('status', $staff->status) == '0' ? 'selected' : '' }}>Inactive
                            </option>
                        </select>
                        @error('status')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                ...
                <div class="mt-4">
                    <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                        ...
                        <span class="text-xs text-gray-500 font-normal">Leave empty to keep current image</span>
                    </label>
                    ...
                    <div x-data="{
                        ...
                        handleFiles(selectedFiles) {
                            ...
                            if (validFiles.length > 0) {
                                ...
                                // Reset remove profile picture flag if new file is uploaded
                        ...
                        <div @drop.prevent="handleDrop($event)" @dragover.prevent="isDragging = true"
                            ...
                            <div class="flex flex-col items-center m-0">
                                ...
                                <h4 class="mb-3 font-semibold text-gray-800 text-theme-xl dark:text-white/90">
                                    <span x-show="!isDragging">Drag & Drop New Profile Picture</span>
                        ...
                        <div x-show="files.length > 0" class="mt-4 p-4 border-t border-gray-200 dark:border-gray-700"
                            x-cloak>
                            <h5 class="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">New Uploaded File:</h5>
                            <ul class="space-y-2">
