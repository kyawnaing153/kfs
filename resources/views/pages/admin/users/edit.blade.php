    ...
    <div class="grid grid-cols-1 gap-6 xl:grid-cols-1">
        <x-common.component-card title="User Information">
            ...
            <div class="flex items-center justify-between mb-6">
                <div>
                    <h2 class="text-sm font-semibold text-gray-500 dark:text-white/90">
                        Edit User: {{ $user->name }}
                    </h2>
            ...
            <form method="POST" action="{{ route('users.update', $user->id) }}" enctype="multipart/form-data">
                ...
                <!-- Row 3: Profile Picture, Role, Status -->
                <div class="mt-4 grid grid-cols-1 gap-4 md:grid-cols-3">
                    ...
                    <div>
                        ...
                        <select name="role"
                            ...
                            <option value="1" {{ old('role', $user->role) == '1' ? 'selected' : '' }}>Admin User</option>
                            <option value="2" {{ old('role', $user->role) == '2' ? 'selected' : '' }}>General User</option>
                        </select>
                        @error('role')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <div class="mb-3">
                            <p class="text-sm text-gray-600 dark:text-gray-400 mb-2">Current Picture:</p>
                            <div class="flex items-center gap-3">
                                <img src="{{ $user->profilePic() }}" alt="{{ $user->name }}"
                                    class="h-16 w-16 rounded-full object-cover border-2 border-gray-300 dark:border-gray-700">
                                <div>
                                    <p class="text-sm text-gray-700 dark:text-gray-300">{{ $user->name }}</p>
                                    <p class="text-xs text-gray-500 dark:text-gray-400">Click to upload new picture</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div x-data="{
                            ...
                            <div @drop.prevent="handleDrop($event)" @dragover.prevent="isDragging = true"
                                ...
                                <div class="flex flex-col items-center m-0">
                                    ...
                                    <h4 class="mb-3 font-semibold text-gray-800 text-theme-xl dark:text-white/90">
                                        <span x-show="!isDragging">Upload New Picture</span>
                                        <span x-show="isDragging" x-cloak>Drop Picture Here</span>
                                    </h4>
                            ...
                            <div x-show="files.length > 0" class="mt-4 p-4 border-t border-gray-200 dark:border-gray-700"
                                x-cloak>
                                <h5 class="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">New Picture:</h5>
                                <ul class="space-y-2">
