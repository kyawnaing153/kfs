        ...
        <div class="max-w-7xl mx-auto px-4 pt-20 lg:pt-24">
            ...
            <div class="invoice-container bg-white p-8 lg:p-10">
                ...
                <div class="flex justify-between items-start mb-8 pb-6 border-b-2 border-orange-500 flex-wrap gap-4">
                    <div class="company-info">
                        <h1 class="text-orange-600 text-2xl lg:text-3xl font-bold mb-2">
                            {{ $settings['companyName'] ?? 'Kyaw Family Scaffolding' }}</h1>
                        <p class="text-gray-600 text-sm">{{ $settings['address'] ?? 'Yangon, Myanmar' }}</p>
                        <p class="text-gray-600 text-sm">Phone: {{ $settings['phone'] ?? '09-428111750' }} | Email:
                            {{ $settings['email'] ?? 'info@kfs-scaffolding.com' }}</p>
                    </div>
                    @php
                        $phones = array_map(
                            'trim',
                            explode(',', $rent->customer->phone_number ?? ($rent->customer->phone ?? '')),
                        );
                    @endphp
                ...
                <div class="overflow-x-auto mb-8">
                    <table class="w-full border-collapse">
                        <thead>
                            <tr>
                                <th
                                    class="bg-orange-600 text-white rounded-l-lg text-left p-3 font-semibold text-sm w-[40%]">
                                    Item Description
                                </th>
                                ...
                                <th
                                    class="bg-orange-600 text-white rounded-r-lg text-right p-3 font-semibold text-sm w-[25%]">
                                    Daily Total (Ks)
                                </th>
                            </tr>
                        </thead>
                                ...
                                <tr class="border-b border-gray-200 hover:bg-gray-50 transition-colors">
                                    <td class="p-3 text-sm">
                                        ...
                                        @if ($item->productVariant->size ?? false)
                                            <p class="text-gray-500 text-xs mt-1">Size: {{ $item->productVariant->size }}
                                            </p>
                                        @endif
                            ...
                            @if (count($rent->items) > 0)
                                <tr class="bg-gray-50">
                                    ...
                                    <td class="p-2 text-sm text-right font-semibold">
                                        {{ number_format($rent->daily_subtotal, 0) }} Ks
                                    </td>
                                </tr>
                            @else
                                <tr>
                                    <td colspan="5"
                                        class="p-2 border-b border-gray-200 text-sm text-center text-gray-500">
                                        No rental items found
                                    </td>
                                </tr>
                            @endif
                    ...
                    <div>
                        ...
                        <div class="bg-gray-50 rounded-lg p-4">
                            ...
                            <ul class="space-y-2 text-sm text-gray-600">
                                <li class="flex gap-2">• <span><strong>Damage/Loss:</strong> Customer is responsible for any
                                        damage or loss of rented items</span></li>
                                <li class="flex gap-2">• <span><strong>Late Return:</strong> Additional charges apply for
                                        late returns at 150% of daily rate</span></li>
                                <li class="flex gap-2">• <span><strong>Inspection:</strong> All items must be inspected upon
                                        return</span></li>
                            </ul>

                            @if ($rent->payment_type == 'kpay')
                                <div class="mt-3 pt-3 border-t border-gray-200">
                                    <p class="text-sm"><span class="font-semibold text-gray-800">K-pay Number:</span>
                                        09428111750</p>
                                </div>
                            @endif
                                    ...
                                    <div class="space-y-1">
                                        @foreach ($rent->payments as $payment)
                                            <p class="text-sm text-gray-600">
                                                ...
                                                <span class="font-semibold">{{ number_format($payment->amount, 0) }}
                                                    Ks</span>
                                ...
                                <tbody>
                                    ...
                                    @if (($rent->total_paid ?? 0) > 0)
                                        ...
                                        <tr class="{{ ($rent->total_due ?? 0) > 0 ? 'bg-red-50' : 'bg-green-50' }}">
                                            <td
                                                class="p-3 font-bold {{ ($rent->total_due ?? 0) > 0 ? 'text-red-600' : 'text-green-600' }}">
                                                {{ ($rent->total_due ?? 0) > 0 ? 'BALANCE DUE' : 'PAID IN FULL' }}
                                            </td>
                                            <td
                                                class="p-3 text-right font-bold {{ ($rent->total_due ?? 0) > 0 ? 'text-red-600' : 'text-green-600' }}">
                ...
                <div class="text-center pt-8 mt-8 border-t border-gray-200">
                    ...
                    <p class="text-gray-500 text-sm">For any inquiries regarding this rental invoice, please contact our
                        customer support.</p>
