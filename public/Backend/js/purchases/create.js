// Purchase Creation Manager - jQuery AJAX Version
(function ($) {
    'use strict';

    // PurchaseCreator class to manage all purchase creation functionality
    function PurchaseCreator() {
        this.itemCount = 0;
        this.selectedProducts = new Set();
        this.productVariants = [];
        this.csrfToken = $('meta[name="csrf-token"]').attr('content');
        this.storeUrl = $('#purchaseForm').attr('action') || '/purchases';
        this.init();
    }

    PurchaseCreator.prototype = {
        // Initialize the purchase creator
        init: function () {
            var self = this;
        ...
        loadProductVariants: function (callback) {
            ...
            $.ajax({
                url: '/admin/purchases/available-variants',
                type: 'GET',
                dataType: 'json',
                headers: {
                ...
                <div id="item-card-${index}" class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4">
                    ...
                    <div class="space-y-3">
                        <div>
                            ...
                            <input type="hidden" name="items[${index}][product_id]" class="product-id-input" data-index="${index}">
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                <label class="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-400">
                                    Receive Quantity<span class="text-red-500">*</span>
                                </label>
                                <input type="number" name="items[${index}][received_qty]" min="1" value="1"
                            ...
                            <div>
                                ...
                                <input type="number" name="items[${index}][unit_price]" step="0.1" min="0"
        ...
        getProductOptions: function () {
            ...
            $.each(this.productVariants, function (i, variant) {
                ...
                if (variant) {
                    ...
                    var price = 0;
                    var productId = variant.product_id || (variant.product ? variant.product.id : null);
        ...
        updateProductInfo: function (index, element) {
            ...
            if (option.val()) {
                ...
                // Update hidden product_id input
                $('.product-id-input[data-index="' + index + '"]').val(productId);
        ...
        calculateTotals: function () {
            ...
            // Calculate totals
            var grandTotal = subTotal + transport - discount + tax;

            // Update displays
            $('#displaySubTotal').text('$' + subTotal.toFixed(1));
            $('#total_amount').text('$' + grandTotal.toFixed(1));
        ...
        bindEvents: function () {
            ...
            $(document).on('input', '#discount, #transport, #tax',function () {
                // If transport
                var transport = parseFloat($('#transport').val()) || 0;
                self.calculateTotals();
            });
        ...
        validateForm: function () {
            ...
            if (!hasItems) {
                this.showError('Please add at least one item to the purchase.');
                return false;
            }
            ...
            if (!purchaseDate) {
                this.showError('Please select a purchase date.');
                $('#purchase_date').focus();
                return false;
            }
                ...
                $('#purchaseForm').append(`
                    <div id="formProcessing" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
                        <div class="bg-white dark:bg-gray-800 rounded-lg p-6 text-center">
                            ...
                            <p class="text-gray-700 dark:text-gray-300">Creating purchase...</p>
    ...
    $(document).ready(function () {
        // Check if we're on the purchase creation page
        if ($('#purchaseForm').length) {
            window.purchaseCreator = new PurchaseCreator();

            // Optional: Initialize product search
            // window.purchaseCreator.initProductSearch();
        }
    });
